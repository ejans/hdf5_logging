...
#include "hdf5_logger.lua.hexarr"
#else
#define FILE_LOG_FILE "/home/mk/prog/c/microblx/std_blocks/reporter/hdf5_logger.lua"
#endif

char hdf5_logger_meta[] =
	"{ doc='A reporting block that writes to a hdf5 file',"
	"  license='MIT',"
	"  real-time=false,"
	"}";

ubx_config_t hdf5_logger_conf[] = {
...
{
	int ret = 0;
	struct hdf5_logger_info* inf = (struct hdf5_logger_info*) b->private_data;
 ...
 */
static int init_lua_state(struct hdf5_logger_info* inf)
{
	int ret=-1;
...
#ifdef COMPILE_IN_LOG_LUA_FILE
	ret = luaL_dostring(inf->L, (const char*) &hdf5_logger_lua);
#else
	ret = luaL_dofile(inf->L, FILE_LOG_FILE);
#endif
	
	if (ret) {
		ERR("Failed to load hdf5_logger.lua: %s\n", lua_tostring(inf->L, -1));
		goto out;
	}
	ret=0;
...
static int hdf5_logger_init(ubx_block_t *b)
{
	DBG(" ");
	int ret = -EOUTOFMEM;
	struct hdf5_logger_info* inf;

	if((inf = calloc(1, sizeof(struct hdf5_logger_info)))==NULL)
		goto out;
...
static int hdf5_logger_start(ubx_block_t *b)
{
	DBG(" ");
...
/**
 * hdf5_logger_step - execute lua string and call step hook
 *
 * @param b
 */
static void hdf5_logger_step(ubx_block_t *b)
{
	call_hook(b, "step", 0, 0);
	return;
}

static void hdf5_logger_stop(ubx_block_t *b)
{
	call_hook(b, "stop", 0, 0);
}

static void hdf5_logger_cleanup(ubx_block_t *b)
{
	struct hdf5_logger_info* inf = (struct hdf5_logger_info*) b->private_data;
...
ubx_block_t lua_comp = {
	.name = "hdf5_logging/hdf5_logger",
	.type = BLOCK_TYPE_COMPUTATION,
	.meta_data = hdf5_logger_meta,
	.configs = hdf5_logger_conf,
	/* .ports = lua_ports, */

	/* ops */
	.init = hdf5_logger_init,
	.start = hdf5_logger_start,
	.step = hdf5_logger_step,
	.stop = hdf5_logger_stop,
	.cleanup = hdf5_logger_cleanup,
};

static int hdf5_logger_mod_init(ubx_node_info_t* ni)
{
	return ubx_block_register(ni, &lua_comp);
}

static void hdf5_logger_mod_cleanup(ubx_node_info_t *ni)
{
	ubx_block_unregister(ni, "hdf5_logging/hdf5_logger");
}

UBX_MODULE_INIT(hdf5_logger_mod_init)
UBX_MODULE_CLEANUP(hdf5_logger_mod_cleanup)
