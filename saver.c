...
#include "saver.lua.hexarr"
#else
#define FILE_LOG_FILE "/home/mk/prog/c/microblx/std_blocks/hdf5_logging/saver.lua"
#endif

char saver_meta[] =
	"{ doc='A block that gets an hdf5 file and writes it to disk',"
	"  license='MIT',"
	"  real-time=false,"
	"}";

ubx_config_t saver_conf[] = {
...
ubx_port_t saver_ports[] = {
	{ .name="data", .attrs=PORT_DIR_IN, .out_type_name="struct hdf5_logging_data" },
	{ NULL },
};
...
{
	int ret = 0;
	struct saver_info* inf = (struct saver_info*) b->private_data;
 ...
 */
static int init_lua_state(struct saver_info* inf)
{
	int ret=-1;
...
#ifdef COMPILE_IN_LOG_LUA_FILE
	ret = luaL_dostring(inf->L, (const char*) &saver_lua);
#else
	ret = luaL_dofile(inf->L, FILE_LOG_FILE);
#endif
	
	if (ret) {
		ERR("Failed to load saver.lua: %s\n", lua_tostring(inf->L, -1));
		goto out;
	}
	ret=0;
...
static int saver_init(ubx_block_t *b)
{
	DBG(" ");
	int ret = -EOUTOFMEM;
	struct saver_info* inf;

	if((inf = calloc(1, sizeof(struct saver_info)))==NULL)
		goto out;
...
static int saver_start(ubx_block_t *b)
{
	DBG(" ");
...
/**
 * saver_step - execute lua string and call step hook
 *
 * @param b
 */
static void saver_step(ubx_block_t *b)
{
	call_hook(b, "step", 0, 0);
	return;
}

static void saver_stop(ubx_block_t *b)
{
	call_hook(b, "stop", 0, 0);
}

static void saver_cleanup(ubx_block_t *b)
{
	struct saver_info* inf = (struct saver_info*) b->private_data;
...
ubx_block_t lua_comp = {
	.name = "hdf5_logging/saver",
	.type = BLOCK_TYPE_COMPUTATION,
	.meta_data = saver_meta,
	.configs = saver_conf,
	/* .ports = lua_ports, */
	.ports = saver_ports,
	...
	.step = saver_step,
	.stop = saver_stop,
	.cleanup = saver_cleanup,
};

static int saver_mod_init(ubx_node_info_t* ni)
{
	return ubx_block_register(ni, &lua_comp);
}

static void saver_mod_cleanup(ubx_node_info_t *ni)
{
	ubx_block_unregister(ni, "hdf5_logging/saver");
}

UBX_MODULE_INIT(saver_mod_init)
UBX_MODULE_CLEANUP(saver_mod_cleanup)
